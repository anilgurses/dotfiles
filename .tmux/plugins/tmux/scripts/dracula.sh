...
main()
{
  ...
  for plugin in "${plugins[@]}"; do
    ...
    if [ $plugin = "spotify" ]; then
      IFS=' ' read -r -a colors <<<$(get_tmux_option "@dracula-spotify-tui-colors" "green dark_gray")
      script="#($current_dir/spotify-dbus.sh)"
    fi
    ...
    if [ $plugin = "wakatime" ]; then
      IFS=' ' read -r -a colors <<< $(get_tmux_option "@dracula-wakatime-colors" "yellow dark_gray")
      script="#($current_dir/waka_time.sh)"
    fi
